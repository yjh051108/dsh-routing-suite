...
/** 模式名/判定法/验收重心一句话（家政平台适配：判定法/重心追加平台验收实践，name 保持通用——平台事实见 AGENTS.md/CLAUDE.md）。 */
const MODES = {
  correct: {
    name: '正确性（C）',
    judge: '主体是代码/数据/接口/公式/可测量的行为（家政平台：端到端真实联调为唯一验证——无门禁/契约测试，证据必须针对真实 CloudBase 与真实产物，禁止用模拟成功冒充真联调；主端禁止 uni-app/HBuilderX/DCloud/Express 栈）',
    center: '证据链与指标：概念→证据(数值)→结论（家政平台证据=真实联调回执/四端包构建数值/小程序预览截图，非模拟数据）',
  },
  experience: {
    name: '体验（E）',
    judge: '主体是视觉/交互/氛围/叙事/手感——最终是"给人用的"（家政平台：四端看与玩——customer-app/worker-app 双 APK、admin/dispatch 双 H5、用户端小程序，体感先行）',
    center: '体感验收：亲眼看/亲手玩/亲耳听，感受判断先于指标（家政平台：四端实机/浏览器/预览画面为第一证据）',
  },
  research: {
    name: '研究（R）',
    judge: '主体是分析/论证/调查/比较——最终是"结论可信"（家政平台：以 docs/decisions、AGENTS.md/CLAUDE.md 与 docs/architecture-lab/contracts 为权威来源）',
    center: '可复核性：每条结论有来源、推导链与复现路径（家政平台：复现路径=仓库内文档行号/决策记录/静态契约，非网络猜测）',
  },
}
...
基准题集（可裁剪/合并；**含必选模式题**）：
· 本任务验收模式（**必选**）：correct=代码/数据/接口/可测量行为（家政平台：端到端真联调、无门禁，证据针对真实 CloudBase 与四端产物）/ experience=视觉/交互/氛围/手感（家政平台：四端看与玩）/ research=分析/论证/调查（家政平台：文档与决策权威源可复核）（单选）
· 范围/对象：默认=…，对吗？（单选）
· 完成度：底线可判 vs 理想态另列（单选）
· 约束/环境：资源/时机/技术取舍（多选）
· 平台特化（家政平台专项）：任务是否触及主航道（四端构建/用户端小程序预览/真实 CloudBase 联调/双轨同步）？涉及平台的改动按 AGENTS.md 端到端真联调验证，还是纯工具链/DSH 侧任务（单选）
· 出口/收口：链尾如何收（单选）
